...
jest.mock('../src/playwrightScraper', () => ({
  fetchNotamsWithPlaywright: jest.fn().mockResolvedValue([
    {
      ...
      isExpanded: true,
      expandedContent: 'Expanded mock content with dates FROM 2501031200 TO 2501031800'
    }
  ])
}));
...
jest.mock('../src/scraper', () => ({
  fetchNotams: jest.fn().mockResolvedValue('<html><body>Mock HTML</body></html>'),
...
describe('Parser Integration', () => {
  ...
  describe('fetchAndParseNotamsIncremental', () => {
    ...
    it('should fail if Playwright fails (no fallback for safety)', async () => {
      ...
      await expect(fetchAndParseNotamsIncremental(true)).rejects.toThrow('Playwright failed');
    }, 10000);
  });
