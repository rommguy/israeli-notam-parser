...
const { PlaywrightNotamScraper } = require("./dist/scraper");
...
async function testRealScraping() {
  ...
  try {
    ...
    console.log(
      "   (Browser window should be visible - you can watch the process)\n"
    );
    ...
    notams.slice(0, 3).forEach((notam, index) => {
      ...
      if (notam.expandedContent) {
        console.log(
          `   Expanded Content Length: ${notam.expandedContent.length} chars`
        );
      }
    });
    ...
    console.log(
      `   Successfully Expanded: ${notams.filter((n) => n.isExpanded).length}`
    );
    console.log(
      `   With Valid Dates: ${notams.filter((n) => n.validFrom && n.validTo).length}`
    );
  } catch (error) {
